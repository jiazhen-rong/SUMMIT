...
# RCTD ratio
rctd_epi = readRDS(paste0("~/nzhanglab/project/jrong/mito_LT/results/Sydney_Bracht/sample_level/EFF3/RCTD/snRNA/RCTD_res.rds"))
Ws=as.data.frame(as.matrix(normalize_weights(rctd_epi@results$weights))) 
epi_celltypes = c("Enterocytes","Enteroendocrine","Goblet cells",
                  "Intestinal stem cells","Transit-amplifying cells","Tuft")
Ws$Epi = rowSums(Ws[,epi_celltypes])
Ws$Non_Epi = rowSums(Ws[,!(colnames(Ws) %in% c(epi_celltypes,"Epi"))]) 
Ws = Ws[,c("Epi","Non_Epi")]
celltypes = c("Epi","Non_Epi")
Ws = Ws[colnames(af.dm),] 
deconv=rctd_epi

#deconv=as.matrix(deconv)
# create niche by VOI:
maegatk.rse = readRDS("../../data/Sydney_Bracht/EFF3/MAESTER/run1run2_comb/rds/EEF3_maegatk_final.rds")
...
niche = as.matrix(Matrix(0,nrow=dim(seu)[2],ncol=3)) # X covariate matrix
rownames(niche) = colnames(seu);colnames(niche)=c(voi ,"Rest")
niche[colnames(af.dm)[af.dm["9811_G>A",]>0.25],"9811_G>A"]=1
niche[colnames(af.dm)[af.dm["9797_T>C",]>0.25],"9797_T>C"]=1
niche[(niche[,"9811_G>A"]==0) & (niche[,"9797_T>C"]==0)  ,
        "Rest"] = 1
...
saveRDS(res,"~/nzhanglab/project/jrong/mito_LT/scripts/SpotGLM/EFF3/spotglm_res.rds")


res <- readRDS("~/nzhanglab/project/jrong/mito_LT/scripts/SpotGLM/P3A2/spotglm_res.rds")
...
saveRDS(all_contrasts,"~/nzhanglab/project/jrong/mito_LT/scripts/SpotGLM/EFF3/deg_2sided.rds")

all_contrasts <- readRDS("~/nzhanglab/project/jrong/mito_LT/scripts/SpotGLM/EFF3/deg_2sided.rds")
# remove MT genes
...
pdf_path="~/nzhanglab/project/jrong/mito_LT/scripts/SpotGLM/EFF3/celltype_deg_top20_2sided_MTremoved.pdf"
